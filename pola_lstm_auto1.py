#!/usr/bin/env python3
"""
pola_lstm_auto.py — Aviator LSTM + Binary Decision + Auto-Loop (FIXED)
Author: Malcu v3
Date: 2026-06-18
Fix: Konversi semua data live ke float untuk menghindari TypeError.
"""
...
# ========== KONFIGURASI ==========
DATA_DIR = "/home/muhammad194494/aviator"          # ganti ke folder yang lo pakai di Cloud Shell
...
def load_all_sessions(data_dir):
    ...
    def sort_key(fname):
        ...
        if base == "aviator_data.json":
            return (1, 999)
        return (2, 1000)  # aviator_data_live.json di akhir
    ...
    for f in files:
        try:
            ...
            sessions.append(np.array([float(x) for x in data], dtype=np.float32))
...
# ──────────────────────────────────────────
#  MODE AUTO‑LOOP (FIX: konversi float)
# ──────────────────────────────────────────
def auto_prediction_loop(model, dmin, dmax):
    ...
    while True:
        ...
        try:
            ...
            # Pastikan semua elemen menjadi float
        ...
        while len(new_data) >= SEQ_LEN + 1:
            inputs = new_data[:SEQ_LEN]                # sudah float
            actual = new_data[SEQ_LEN]                 # sudah float
