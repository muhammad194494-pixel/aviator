#!/usr/bin/env python3
"""
decision_support_v2.py — Dynamic Decision Support System (Revisi)
Author: Malcu v3
Date: 2026-06-18
...
def analyze(logs):
    ...
    # 5. Ada input di atas 50x (ekstrem) — perbaikan nama
    subset = [l for l in logs if any(v > 50 for v in l[0])]
    if len(subset) >= 5:
        benar = sum(1 for _, _, _, bc in subset if bc)
        rules["Ada input > 50.0x"] = {
            "total": len(subset),
            "benar": benar,
            "akurasi": benar / len(subset)
        }
...
def rekomendasi(inputs, rules):
    ...
    for rule_name, stats in rules.items():
        ...
        if rule_name.startswith("Rata-rata input <"):
            ...
            if max(inputs) < 3.0:
                cocok = True
        elif rule_name == "Ada input > 50.0x":
