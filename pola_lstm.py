#!/usr/bin/env python3
"""
pola_lstm.py — Aviator Pattern‑Aware LSTM + Binary Analyzer (>2x)
Update: Opsi 3 - Learning Rate & Arsitektur Lebih Kecil
"""
...
MODEL_PATH = os.path.join(DATA_DIR, "pola_lstm.keras")
META_PATH = os.path.join(DATA_DIR, "pola_lstm_meta.json")
LOG_FILE = os.path.join(DATA_DIR, "prediction_log.txt")

SEQ_LEN = 4
EPOCHS = 200          # Ditambah supaya belajar lebih lama
BATCH = 32
LR = 0.001            # Diturunkan supaya lebih teliti
SPLIT = 0.8
...
def build_model(input_shape):
    ...
    model = Sequential([
        LSTM(64, return_sequences=True, input_shape=input_shape),   # 128 → 64
        BatchNormalization(),
        Dropout(0.3),
        LSTM(32, return_sequences=False),                          # 64 → 32
        BatchNormalization(),
        Dropout(0.3),
...
def train_model(sessions):
    ...
    es = EarlyStopping(monitor='val_loss', patience=35, restore_best_weights=True, verbose=1)  # 25 → 35
...
def main():
    print("="*50)
    print("🎲 POLA LSTM — Binary Analyzer (Learning Rate 0.001)")
    print("="*50)
